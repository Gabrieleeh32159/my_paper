...
en contextos con recursos computacionales limitados donde el \textit{deep learning} no es viable. Sin embargo, la ventana temporal sobre la cual se agregan estas \textit{features} se elige típicamente por convención --- usualmente 1\,s o la duración completa del clip --- sin justificación acústica ni evidencia empírica  comparativa\textcolor{black}{.} Este trabajo introduce el \textbf{Índice de Sensibilidad Temporal (TSI)}, una métrica que cuantifica 
cuánto varía la discriminabilidad de cada descriptor según la escala temporal de extracción, y lo utiliza para construir estrategias de fusión informadas por datos. Extraemos siete descriptores canónicos a tres 
...
donde $k^*(f)= \arg\max_k \text{Acc}(f,k)$ es la escala óptima para el descriptor $f$ según su TSI, y $\hat{p}_k$ es la probabilidad predicha por el clasificador entrenado en la escala $k$.
...
El TSI es la contribución metodológica central de este trabajo. Para cada descriptor $f \in \{\text{MFCCs, Chroma, SC, SContrast, SR, ZCR, Tonnetz}\}$, definimos:

\begin{equation}
    \text{TSI}(f) = \frac{\max_{k \in \{s,m,l\}} \text{Acc}(f, k) - 
    \min_{k \in \{s,m,l\}} \text{Acc}(f, k)}{\text{Acc}_{\text{chance}}}
    \label{eq:tsi}
\end{equation}

\noindent donde $\text{Acc}(f, k)$ es la precisión de clasificación obtenida usando 
únicamente el descriptor $f$ extraído a la escala $k$, y 
$\text{Acc}_{\text{chance}} = 1/C$ es la precisión de nivel aleatorio para $C$ clases.

La normalización por $\text{Acc}_{\text{chance}}$ hace al TSI comparable entre tareas con diferente número de clases. Un $\text{TSI}(f) = 0$ indica que el descriptor $f$ rinde igual en todas las escalas (\textit{temporalmente robusto}); un TSI alto indica que la discriminabilidad del descriptor depende fuertemente de la escala de extracción (\textit{temporalmente sensible}).

El TSI se computa por separado para cada tarea y clasificador. La consistencia del TSI entre tareas y clasificadores se evalúa mediante el coeficiente de correlación de Spearman $\rho$ entre los rankings de TSI obtenidos en diferentes configuraciones, con $\rho > 0.7$ como umbral para 
afirmar consistencia.

Los resultados se visualizan como una \textbf{matriz de sensibilidad $7 \times 3$} (descriptores $\times$ escalas) donde cada celda contiene $\text{Acc}(f, k)$ normalizada, permitiendo identificar visualmente qué descriptor se beneficia de qué escala.
...
\begin{itemize}
    \item \textbf{Random Forest (RF):} 500 árboles, profundidad máxima 30. Proporciona importancia de \textit{features} integrada mediante \textit{mean decrease in impurity} (MDI). Es el clasificador principal para el cómputo del TSI por su robustez ante \textit{features} de alta dimensión y su capacidad nativa de medir 
    importancia sin costo adicional.

    \item \textbf{Support Vector Machine (SVM):} Kernel RBF, hiperparámetros ($C$, $\gamma$) ajustados mediante validación cruzada de 3 folds en el conjunto de entrenamiento. Para la representación 
    \textit{early fusion} ($\mathbb{R}^{576}$), el entrenamiento se realiza con submuestreo aleatorio del 50\% de las pistas de entrenamiento en MTAT para mantener tiempos de cómputo manejables, reportando este 
    compromiso explícitamente.

    \item \textbf{Multi-Layer Perceptron (MLP):} Dos capas ocultas (256, 128), activación ReLU, \textit{dropout} 0.3 y regularización L2 ($\lambda = 10^{-4}$), entrenado por 100 épocas con 
    Adam ($\text{lr} = 10^{-3}$) y \textit{early stopping} (paciencia 10).
    Para MTAT, la función de pérdida es \textit{weighted binary cross-entropy}; para las demás tareas, \textit{cross-entropy} 
    estándar.
\end{itemize}
...
Todas las comparaciones entre representaciones \textit{single-scale} y \textit{multi-scale} se evalúan para significancia estadística usando la prueba de rangos con signo de Wilcoxon ($\alpha = 0.05$), con corrección de Bonferroni para comparaciones múltiples ($\alpha_{\text{corr}} = 0.05 / 6 = 0.0083$ para las seis comparaciones entre pares de representaciones), aplicada a la \textit{accuracy} por fold 
(GTZAN) o al AP por \textit{tag} (MTAT e IRMAS).
Los tamaños de efecto se reportan usando delta de Cliff con los umbrales estándar: pequeño $|\delta| \geq 0.147$, mediano $|\delta| \geq 0.33$, grande $|\delta| \geq 0.474$ \cite{romano2006exploring}.
Adicionalmente, la consistencia de los rankings de TSI entre tareas se evalúa con el coeficiente de correlación de Spearman $\rho$, reportando el intervalo de confianza del 95\% mediante \textit{bootstrap}.
