...
El \textbf{Índice de Sensibilidad Temporal} se define como la desviación estándar de $G$ entre escalas:

\begin{equation}
    \text{TSI}(f) = \sqrt{\frac{1}{K}\sum_{k=1}^{K}\bigl(G(f,k) - \bar{G}(f)\bigr)^{2}}
    \label{eq:tsi}
\end{equation}

\noindent donde $\bar{G}(f) = \frac{1}{K}\sum_k G(f,k)$ es la ganancia media del descriptor $f$ entre las $K=3$ escalas.

Un $\text{TSI}(f) = 0$ indica que el descriptor rinde igual en todas las escalas (\textit{temporalmente robusto}); un TSI alto indica que la discriminabilidad varía con la escala (\textit{temporalmente sensible}).
A diferencia de un índice basado en rango, esta formulación utiliza las $K$ escalas conjuntamente: dos descriptores con idéntico rango pero distintos perfiles intermedios obtienen valores de TSI diferentes, capturando así descriptores con sensibilidad difusa frente a descriptores con un único salto concentrado entre dos escalas.
La cota natural es $\text{TSI}(f) \in [0,\, \tfrac{1}{2}\,\text{rango}(G)]$, con el mínimo cuando el punto intermedio es equidistante a los extremos.
